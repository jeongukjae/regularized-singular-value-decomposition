...
def main():
    ...
    for epoch_number in range(args.epoch):
        for step_number in tqdm(range(steps_per_epoch), total=steps_per_epoch):
...
def read_dataset(movie_filename: str, ratings_filename: str, delimiter: str) -> Tuple[Dict[int, Movie], List[Rating]]:
    if not os.path.isfile(movie_filename):
        raise ValueError(f"Cannot get movie file {movie_filename}")

    if not os.path.isfile(ratings_filename):
        raise ValueError(f"Cannot get rating file {ratings_filename}")

    def parse_movie(line: str) -> Tuple[int, Movie]:
    ...
    with open(movie_filename) as movie_file:
        movies = dict([parse_movie(line) for line in tqdm(movie_file)])
    ...
    with open(ratings_filename) as ratings_file:
        ratings = [parse_rating(line) for line in tqdm(ratings_file)]
